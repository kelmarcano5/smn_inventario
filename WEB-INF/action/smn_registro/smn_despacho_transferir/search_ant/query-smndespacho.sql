select
	...
	case
	when smn_inventario.smn_caracteristica_almacen.cal_clase_almacen='ST' then '${lbl:b_satelite}'
	when smn_inventario.smn_caracteristica_almacen.cal_clase_almacen='SA' then '${lbl:b_subwarehouse}'
	when smn_inventario.smn_caracteristica_almacen.cal_clase_almacen='IT' then '${lbl:b_itinerant}'
	when smn_inventario.smn_caracteristica_almacen.cal_clase_almacen='PR' then '${lbl:s_main_menu}'
	end as clase_almacen,
...
from
	...
	left outer join smn_base.smn_almacen S on S.smn_almacen_id = smn_inventario.smn_despacho.smn_almacen_solicitante
	
where smn_base.smn_modulos.mod_codigo='SMN_COM'
	and 
	smn_seguridad.s_user.userlogin='${def:user}'
	and smn_inventario.smn_caracteristica_almacen.cal_clase_almacen='ST'
