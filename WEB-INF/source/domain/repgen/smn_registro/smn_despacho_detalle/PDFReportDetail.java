package domain.repgen.smn_proceso.smn_despacho_detalle;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("dde_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("dde_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("dde_estatus_transaccion");
			if (d != null)
			{
			name  = "b_transaction_status";
			label = "${lbl:b_transaction_status}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
