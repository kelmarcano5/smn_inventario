package domain.repgen.smn_configuracion.smn_despacho;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_unidad_organizativa_rf");
			if (d != null)
			{
			name  = "b_unidad_organizativa_rf";
			label = "${lbl:b_unidad_organizativa_rf}";
		...
		d = rs.getString("des_fecha_registro_fin");
			if (d != null)
			{
		...
		d = rs.getString("des_numero_documento");
			if (d != null)
			{
			name  = "b_numero_documento";
			label = "${lbl:b_numero_documento}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("des_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date}@{\${def:date}";
			label = "${lbl:b_store_date}@{\${def:date}}";
		...
		d = rs.getString("smn_clase_auxiliar_rf");
			if (d != null)
			{
			name  = "b_clase_auxiliar_rf";
			label = "${lbl:b_clase_auxiliar_rf}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("des_numero_documento_origen");
			if (d != null)
			{
			name  = "b_numero_documento_origen";
			label = "${lbl:b_numero_documento_origen}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
