package domain.repgen.smn_proceso.smn_movimiento_detalle;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("mde_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("mde_es_bonificacion");
			if (d != null)
			{
			name  = "b_bonus";
			label = "${lbl:b_bonus}";
		...
		d = rs.getString("mde_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("smn_movimiento_cabecera_id");
			if (d != null)
			{
			name  = "b_movement_head";
			label = "${lbl:b_movement_head}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("mde_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
