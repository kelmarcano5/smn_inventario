package domain.repgen.smn_configuracion.smn_transporte;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("tra_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("tra_tipo_transporte");
			if (d != null)
			{
			name  = "b_transport_type";
			label = "${lbl:b_transport_type}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("tra_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("tra_descripcion_transporte");
			if (d != null)
			{
			name  = "b_description";
