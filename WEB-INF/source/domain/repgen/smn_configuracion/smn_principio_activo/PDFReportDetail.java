package domain.repgen.smn_configuracion.smn_principio_activo;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("pac_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("pac_fehca_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("pac_fehca_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("pac_descripcion_completa");
			if (d != null)
			{
