package domain.repgen.smn_proceso.smn_ubicacion_detalle;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_subdivisiones_id");
			if (d != null)
			{
			name  = "b_subdivisions";
			label = "${lbl:b_subdivisions}";
		...
		d = rs.getString("smn_divisiones_id_name");
			if (d != null)
			{
			name  = "b_divisions";
			label = "${lbl:b_divisions}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_divisiones_id");
			if (d != null)
			{
			name  = "b_divisions";
			label = "${lbl:b_divisions}";
		...
		d = rs.getString("ubd_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_subdivisiones_id_name");
			if (d != null)
			{
			name  = "b_subdivisions";
			label = "${lbl:b_subdivisions}";
		...
		d = rs.getString("ubd_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("smn_lote_id_name");
			if (d != null)
			{
			name  = "b_lot";
			label = "${lbl:b_lot}";
