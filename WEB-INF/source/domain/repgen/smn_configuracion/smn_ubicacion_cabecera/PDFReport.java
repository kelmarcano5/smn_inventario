package domain.repgen.smn_configuracion.smn_ubicacion_cabecera;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("ubc_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_movimiento_detalle_id");
			if (d != null)
			{
			name  = "b_detail_movement";
			label = "${lbl:b_detail_movement}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("ubc_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date}@{\${def:date}";
			label = "${lbl:b_store_date}@{\${def:date}}";
