package domain.repgen.smn_registro.smn_control_lote;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("col_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("smn_caracteristica_almacen_id_name");
			if (d != null)
			{
			name  = "b_feature_store";
			label = "${lbl:b_feature_store}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_caracteristica_item_id");
			if (d != null)
			{
			name  = "b_item";
			label = "${lbl:b_item}";
		...
		d = rs.getString("col_fecha_vencimiento");
			if (d != null)
			{
			name  = "b_expiration_date";
			label = "${lbl:b_expiration_date}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_caracteristica_almacen_id");
			if (d != null)
			{
			name  = "b_feature_store";
			label = "${lbl:b_feature_store}";
		...
		d = rs.getString("col_cantidad_final");
			if (d != null)
			{
			name  = "b_final_quantity";
			label = "${lbl:b_final_quantity}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_caracteristica_item_id_name");
			if (d != null)
			{
			name  = "b_item";
			label = "${lbl:b_item}";
		...
		d = rs.getString("col_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date}@{\${def:date}";
			label = "${lbl:b_store_date}@{\${def:date}}";
		...
		d = rs.getString("col_cantidad_inicial");
			if (d != null)
			{
			name  = "b_initial_quantity";
			label = "${lbl:b_initial_quantity}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
